...
module.exports = function (dir, cb) {
  ...
  const filename = path.join(constants.DOTBC_PLUGIN_DIR, dir, EXPECTED_MAIN_FILE_NAME);
  ...
  fs.exists(filename, (exists) => {
    ...
    // plugins are loaded via the dotbc plugin manager utility, which provides a shim
    // and sets up our IPC communications channel
    const childProcess = spawn(constants.DOTBC_PLUGIN_MANAGER_CMD, [filename], {
      cwd: path.join(constants.DOTBC_PLUGIN_DIR, dir),
      NODE_PATH: path.join(constants.DOTBC_PLUGIN_DIR, dir, 'node_modules'),
      env,
