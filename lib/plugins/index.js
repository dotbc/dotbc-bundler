...
const debug = require('debug')('dotbc-desktop-plugin')
...
/*
  ...
  at runtime. When a new plugin is recognized, it is loaded using a shim that allows the dotbc-desktop
  process to communicate with the plugin via IPC. Having plugin run in a separate process keeps dotbc-
...
class PluginManager extends EventEmitter {
  ...
  _ensurePluginDirectoryExists () {
    fs.exists(constants.DOTBC_PLUGIN_DIR, (exists) => {
      if ( ! exists) {
        mkdirp(constants.DOTBC_PLUGIN_DIR, (err) => {
  ...
  _loadAllPlugins () {
    var dirs;
    try {
      dirs = fs.readdirSync(constants.DOTBC_PLUGIN_DIR)
      .filter((d) => fs.statSync(path.join(constants.DOTBC_PLUGIN_DIR, d)).isDirectory());
    } catch (err) {
